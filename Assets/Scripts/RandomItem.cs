    ...
    void Start()
    {
        if (PhotonNetwork.LocalPlayer.IsMasterClient) {
            ...
            for (int n = 0; n < 192; n++)
            {
                ...
                PhotonNetwork.Instantiate(Path.Combine(Path.Combine("PhotonPrefabs", "Shelf"), prefabList[RandomArray[n] % 42].name), locationList[n].position, prefabList[RandomArray[n] % 42].transform.rotation, 0);
            }
            ...
            for (int n = 0; n < 36; n++)
            {
                ...
                PhotonNetwork.Instantiate(Path.Combine(Path.Combine("PhotonPrefabs", "Table"), TprefabList[RandomTArray[n] % 12].name), TlocationList[n].position, TprefabList[RandomTArray[n] % 12].transform.rotation, 0);
            }
        }
    }
}
